...
export function LogoUpload({
  ...
  disabled = false,
  label = 'Logo',
  description = 'PNG, JPG, WebP, GIF, atau SVG — maks. 5 MB',
}: LogoUploadProps) {
  ...
  const handleFile = async (file: File | null) => {
    ...
    setUploading(true);
    try {
      const result = await uploadLogo(file, websiteId);
      onChange(result.url);
      setPreview(null);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <div
          className={`flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-default-300 bg-default-50 ${
            displayUrl ? 'border-solid border-default-200' : ''
          }`}
        >
          {displayUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={displayUrl} alt="Preview logo" className="h-full w-full object-contain p-1" />
          ) : (
        ...
        <div className="flex flex-1 flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {uploading ? 'Mengunggah…' : value ? 'Ganti Logo' : 'Pilih File'}
            </button>
          ...
          {value && !uploading && (
            <p className="break-all text-xs text-default-400">{value}</p>
          )}
        </div>
      </div>
    </div>
